...
// [Your existing code remains exactly the same until the end]

// ====== ADD THIS NEW CODE AFTER YOUR EXISTING CODE ======

// Object to store polylines for each layer
const layerPolylines = {};

// Function to create or update polylines for visible layers
function updateLayerPolylines() {
  // Check each overlay layer
  for (const layerName in overlayMaps) {
    const layer = overlayMaps[layerName];
    
    // If layer is visible on map
    if (map.hasLayer(layer)) {
      // Get all marker positions from this layer
      const latLngs = [];
      layer.eachLayer(function(marker) {
        latLngs.push(marker.getLatLng());
      });
      
      // Only create polyline if we have at least 2 markers
      if (latLngs.length >= 2) {
        // If polyline doesn't exist for this layer, create it
        if (!layerPolylines[layerName]) {
          layerPolylines[layerName] = L.polyline(latLngs, {
            color: layerName === 'point A' ? 'red' : 'blue',
            weight: 3,
            opacity: 0.7
          }).addTo(map);
        } else {
          // Update existing polyline with new coordinates
          layerPolylines[layerName].setLatLngs(latLngs);
        }
      }
    } else {
      // If layer is not visible but has a polyline, remove it
      if (layerPolylines[layerName]) {
        map.removeLayer(layerPolylines[layerName]);
        delete layerPolylines[layerName];
      }
    }
  }
}

// Modify the existing layeradd/layerremove event handler to also update polylines
const originalLayerHandler = map._events.layeradd[0].action;
map.off('layeradd layerremove');

map.on('layeradd layerremove', function(e) {
  originalLayerHandler.call(this, e); // Call the original handler
  updateLayerPolylines(); // Update our polylines
});

// Initial update of polylines (in case any layers are visible by default)
updateLayerPolylines();
