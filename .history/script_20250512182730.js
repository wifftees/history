...
/**
 * Multiple layers of markers with lines between them
 * Adding the ability to show or hide markers and lines with buttons.
 */
...
const allLines = new L.FeatureGroup(); // to store lines between markers
...
// Create lines between the pointsA
for (let i = 0; i < pointsA.length - 1; i++) {
  const line = L.polyline(
    [[pointsA[i][0], pointsA[i][1]], [pointsA[i + 1][0], pointsA[i + 1][1]]], 
    { color: 'blue', weight: 2 }
  ).addTo(allLines);
}

// Create lines between the pointsB
for (let i = 0; i < pointsB.length - 1; i++) {
  const line = L.polyline(
    [[pointsB[i][0], pointsB[i][1]], [pointsB[i + 1][0], pointsB[i + 1][1]]], 
    { color: 'blue', weight: 2 }
  ).addTo(allLines);
}
...
const overlayMaps = {
  "point A": pA,
  "point B": pB,
  "lines": allLines, // Add lines as an overlay
};
...
L.Control.CustomButtons = L.Control.Layers.extend({
  onAdd: function () {
    this._initLayout();
    this._addShowButton();  // Add show button
    this._addHideButton();  // Add hide button
    this._update();
    return this._container;
  },
  
  _addShowButton: function () {
    this.createButton("show", "show-button");
  },
  
  _addHideButton: function () {
    this.createButton("hide", "hide-button");
  },
  ...
  createButton: function (type, className) {
    ...
    button.textContent = `${type} markers and lines`;

    L.DomEvent.on(button, "click", function (e) {
      if (type === "show") {
        pA.addTo(map);  // Add markers from point A
        pB.addTo(map);  // Add markers from point B
        allLines.addTo(map);  // Add lines between points
      } else {
        pA.removeFrom(map);  // Remove markers from point A
        pB.removeFrom(map);  // Remove markers from point B
        allLines.removeFrom(map);  // Remove lines between points
      }
    });
  },
});
