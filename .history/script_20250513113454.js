...
for (let i = 0; i < pointsB.length; i++) {
  let marker = L.marker([pointsB[i][0], pointsB[i][1]]).bindPopup(pointsB[i][2]);
  pB.addLayer(marker);
}
...
L.Control.CustomButtons = L.Control.Layers.extend({
  ...
  _addA: function () {
    this.createButton("add", "add-a-button", pA, arrowsA);
  },
  
  _addB: function () {
    this.createButton("add", "add-b-button", pB, arrowsB);
  },
  _removeA: function () {
    this.createButton("remove", "remove-a-button", pA, arrowsA);
  },
  createButton: function (type, className, pointLayer, arrowLayer) {
    ...
    button.textContent = `${type} markers and arrows`;
