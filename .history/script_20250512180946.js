...
// Variables to store polylines
const polylines = {
  pA: null,
  pB: null
};

// Function to create a polyline for a layer
function createPolyline(layer, color) {
  const latLngs = [];
...
for (let i = 0; i < pointsA.length; i++) {
  const marker = L.marker([pointsA[i][0], pointsA[i][1]]).bindPopup(pointsA[i][2]);
  pA.addLayer(marker);
}

// adding markers to the layer pointsB
for (let i = 0; i < pointsB.length; i++) {
  const marker = L.marker([pointsB[i][0], pointsB[i][1]]).bindPopup(pointsB[i][2]);
  pB.addLayer(marker);
}
...
// Update polylines when layers are added/removed
function updatePolylines() {
  if (map.hasLayer(pA)) {
    removePolyline(polylines.pA);
    polylines.pA = createPolyline(pA, 'red');
  } else if (polylines.pA) {
    removePolyline(polylines.pA);
    polylines.pA = null;
  }
  
  if (map.hasLayer(pB)) {
    removePolyline(polylines.pB);
    polylines.pB = createPolyline(pB, 'blue');
  } else if (polylines.pB) {
    removePolyline(polylines.pB);
    polylines.pB = null;
  }
}
